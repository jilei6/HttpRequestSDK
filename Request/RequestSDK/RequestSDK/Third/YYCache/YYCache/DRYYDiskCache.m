...
static int64_t _DRYYDiskSpaceFree() {
    NSError *error = nil;
...
/// String's md5 hash.
static NSString *_DRYYNSStringMD5(NSString *string) {
...
static void _DRYYDiskCacheInitGlobal() {
...
static DRYYDiskCache *_YYDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _DRYYDiskCacheInitGlobal();
...
static void _DRYYDiskCacheSetGlobal(DRYYDiskCache *cache) {
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _DRYYDiskSpaceFree();
...
- (NSString *)_filenameForKey:(NSString *)key {
    ...
    if (!filename) filename = _DRYYNSStringMD5(key);
    return filename;
}
...
- (instancetype)initWithPath:(NSString *)path
    ...
    DRYYDiskCache *globalCache = _YYDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    DRYYKVStorageType type;
    if (threshold == 0) {
        type = DRYYKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = DRYYKVStorageTypeSQLite;
    } else {
        type = DRYYKVStorageTypeMixed;
    }
    
    DRYYKVStorage *kv = [[DRYYKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    DRYYKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [DRYYDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [DRYYDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != DRYYKVStorageTypeSQLite) {
