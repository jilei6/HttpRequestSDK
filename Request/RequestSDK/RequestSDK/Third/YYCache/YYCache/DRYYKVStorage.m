...
static const int DRkPathLengthMax = PATH_MAX - 64;
static NSString *const DRkDBFileName = @"manifest.sqlite";
static NSString *const DRkDBShmFileName = @"manifest.sqlite-shm";
static NSString *const DRkDBWalFileName = @"manifest.sqlite-wal";
static NSString *const DRkDataDirectoryName = @"data";
static NSString *const DRkTrashDirectoryName = @"trash";
...
@implementation DRYYKVStorageItem
@end

@implementation DRYYKVStorage {
...
- (DRYYKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    DRYYKVStorageItem *item = [DRYYKVStorageItem new];
...
- (DRYYKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    DRYYKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            DRYYKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
 ...
 */
- (void)_reset {
    [[NSFileManager defaultManager] removeItemAtPath:[_path stringByAppendingPathComponent:DRkDBFileName] error:nil];
    [[NSFileManager defaultManager] removeItemAtPath:[_path stringByAppendingPathComponent:DRkDBShmFileName] error:nil];
    [[NSFileManager defaultManager] removeItemAtPath:[_path stringByAppendingPathComponent:DRkDBWalFileName] error:nil];
...
- (instancetype)init {
    ...
    return [self initWithPath:nil type:DRYYKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(DRYYKVStorageType)type {
    if (path.length == 0 || path.length > DRkPathLengthMax) {
        NSLog(@"YYKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > DRYYKVStorageTypeMixed) {
    ...
    _path = path.copy;
    _type = type;
    _dataPath = [path stringByAppendingPathComponent:DRkDataDirectoryName];
    _trashPath = [path stringByAppendingPathComponent:DRkTrashDirectoryName];
    _trashQueue = dispatch_queue_create("com.ibireme.cache.disk.trash", DISPATCH_QUEUE_SERIAL);
    _dbPath = [path stringByAppendingPathComponent:DRkDBFileName];
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtPath:path
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error] ||
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:DRkDataDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error] ||
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:DRkTrashDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error]) {
...
- (BOOL)saveItem:(DRYYKVStorageItem *)item {
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == DRYYKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != DRYYKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case DRYYKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case DRYYKVStorageTypeFile:
        case DRYYKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (DRYYKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (DRYYKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    DRYYKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (DRYYKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    DRYYKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
...
- (NSArray *)getItemForKeys:(NSArray *)keys {
    ...
    if (_type != DRYYKVStorageTypeSQLite) {
        for (NSInteger i = 0, max = items.count; i < max; i++) {
            DRYYKVStorageItem *item = items[i];
            if (item.filename) {
                item.value = [self _fileReadWithName:item.filename];
                if (!item.value) {
